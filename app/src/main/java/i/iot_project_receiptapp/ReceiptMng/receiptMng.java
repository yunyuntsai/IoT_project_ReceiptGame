 ...
 */
public class receiptMng {
    private final static String  TAGLOG = "ReceiptHelper";
    public static int parserQRcode(ReceiptList receiptdata ,String data){
        if(data==null)
            return -1;
        if(data.startsWith("**"))
            return -1;
...
//	}
    ...
    public static MonthPeriod getPeriod(int month){
        if(month>=1&&month<=2){
            return MonthPeriod.one_TwoMonth;
        }
        else if(month>=3&&month<=4){
            return MonthPeriod.three_FourMonth;
        }
        else if(month>=5&&month<=6){
            return MonthPeriod.five_SixMonth;
        }
        else if(month>=7&&month<=8){
            return MonthPeriod.seven_EightMonth;
        }
        else if(month>=9&&month<=10){
            return MonthPeriod.nine_TenMonth;
        }
        else {
            return MonthPeriod.eleven_TwelveMonth;
        }
    }
}
